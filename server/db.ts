...
// Out-of-band notifications already delivered to the user's Discord while the
// agent was idle — /api/notify calls, scheduled-job output, bin/notify.
// Drained into the next agent turn's prompt for the conversation they were
// delivered to (`session_key`), so the agent knows what the user has seen
...
export type PendingContextRow = {
  ...
  created_at: number;
  source: string;
  /** What kind of alert this is (e.g. "draft digest", "scheduled job output"). */
  kind: string | null;
  text: string;
  session_key: string;
};
...
/** Per-item cap — the full message went to Discord; the agent only needs a summary. */
 ...
 * (the user saw the alert on Discord regardless).
 */
