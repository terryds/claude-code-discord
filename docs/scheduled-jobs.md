...
the job's **Discord delivery channel**. Read this whole file before writing or
registering one. (The always-on trigger rules live in `AGENTS.md`; this file
is the detail.)

## The delivery channel — required, never assumed

Every job delivers to exactly one Discord channel id (`--channel`): a server
channel, a thread, or the owner's DM channel. **The user must have named the
destination** ("…and tell me in #btcusdalert"). If they didn't say where,
**ask before registering** — never silently pick one. Resolve `#name` → id
with `bin/discord channels <guild_id>` (guild id from `bin/discord guilds`);
"just DM me" → the `discord_dm_channel_id` setting (or
`bin/notify --dry-run` prints nothing useful — read it from the DB:
`sqlite3 data/app.db "SELECT value FROM settings WHERE key='discord_dm_channel_id'"`).
...
bin/job add --name btc-alert --schedule "0 10 * * *" \
  --channel 1234567890123456789 \
  --desc "Alert in #btcusdalert when BTC crosses \$70k (5pm WIB daily)" \
  data/jobs/btc-alert/run.ts          # 3. register
```

## The job contract

- **stdout is the message.** Non-empty stdout (exit 0) is sent to the job's
  Discord channel as Markdown; empty stdout means "nothing to report" and
  sends nothing. Never call `bin/notify` from a job script.
...
- **Failures:** non-zero exit is recorded (visible in `/jobs` and the
  dashboard); after 3 consecutive failures the relay warns the owner's DM
  once, then stays quiet until the job succeeds again.
...
```ts
// data/jobs/btc-alert/run.ts — message only when BTC *crosses* above $70k
const THRESHOLD = 70_000;
const r = await fetch(
  'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd'
);
const price: number = (await r.json()).bitcoin.usd;
const state = await Bun.file(process.env.STATE_FILE!).json().catch(() => ({ above: false }));
const above = price > THRESHOLD;
if (above && !state.above) {
  console.log(`🚀 **Bitcoin crossed $${THRESHOLD.toLocaleString()}** — now $${price.toLocaleString()}`);
}
await Bun.write(process.env.STATE_FILE!, JSON.stringify({ above, price }));
```

## Managing jobs

`bin/job list | run <name> | remove <name> | enable <name> | disable <name>`
(`run` fires it once now and *does* deliver to Discord). The user sees jobs
via the `/jobs` command and the dashboard's Scheduled-jobs card. When the
user asks to stop watching something, `bin/job remove <name>` and delete
...
turn is alive (cron's PATH is nearly empty) and pipe output to
`bin/notify --channel <id>` yourself.
