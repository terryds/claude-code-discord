    ...
    <div className="min-h-full max-w-4xl mx-auto px-6 py-10 space-y-8">
      ...
      <header>
        ...
        <p className="text-zinc-400 text-sm mt-1">
          ...
          anything, just ask over Discord (see below).
        </p>
      </header>
...
/** An example Discord message, styled like a chat bubble. Tap to copy. */
      ...
      <div className="min-w-0 flex-1 space-y-2">
        ...
        {prompts && prompts.length > 0 && (
          <div className="space-y-1.5 pt-1">
            <div className="text-[11px] uppercase tracking-wide text-zinc-500">Say in Discord</div>
            {prompts.map((p) => (
  ...
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-5 space-y-5">
      <div>
        ...
        <p className="text-zinc-400 text-sm mt-1">
          No SSH needed — everything below is done by messaging Claude in Discord. Tap an
          example to copy it.
        </p>
      </div>
      ...
      {missing.length > 0 && (
        <div className="rounded-lg border border-amber-900/60 bg-amber-950/30 px-4 py-3 text-sm text-amber-200 space-y-2">
          <div>
            ...
            — without it Claude can't finish sign-ins from Discord. Run{' '}
      ...
      <div className="space-y-5 divide-y divide-zinc-800 [&>*+*]:pt-5">
        ...
        >
          Claude adds the server and sends you a sign-in link. Open it on your device and approve.
    ...
    <section>
      ...
      {servers.some((s) => s.status === 'needs_auth') && (
        <p className="mt-2 text-xs text-zinc-500">
          ...
          claude.ai → Settings → Connectors. For anything else, ask in Discord: “Sign in to
