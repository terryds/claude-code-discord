...
export default defineConfig({
  ...
  server: {
    ...
    allowedHosts: true,
    proxy: {
      // Mirror the server's default port (8100; falls back to 8101 if taken).
      // Override with PORT if you run the server elsewhere.
      '/api': `http://localhost:${process.env.PORT || 8100}`,
    },
  },
});
